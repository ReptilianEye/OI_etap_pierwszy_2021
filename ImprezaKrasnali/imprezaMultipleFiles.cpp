...
void przygotowujeTabliceDoSprawdzarki(int *skojarzenia)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (mozliweWystapienia[i] != 0)
        {
            int iloscFaktycznychWystapien = 0;
            for (int k = 0; k < n; k++)
            {
                for (int j = 0; j < mozliweOdpowiedzi[k].size(); j++)
                {
                    if (mozliweOdpowiedzi[k][j] == i)
                    {
                        iloscFaktycznychWystapien++;
                        break;
                    }
                }
            }
            mozliweWystapienia[i] = iloscFaktycznychWystapien;
        }
    }
